...
class MLService {
  ...
  private createMockModel() {
    ...
    return {
      predict: (input: any) => {
        ...
        const mockPredictions = new Array(this.labels.length).fill(0).map(() => Math.random());
  ...
  public async predict(imagePath: string): Promise<PredictionResult> {
    console.log('Predict called with:', { isInitialized: this.isInitialized, hasModel: !!this.model });
    ...
    try {
      ...
      const isHealthy = predictedLabel.toLowerCase().includes('healthy') || 
